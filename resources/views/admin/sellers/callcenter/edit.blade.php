...
@section('title')
	Actualizar Vendedor de Call Center
@stop

@section('content')

{!! Form::open(['route' => ['admin.sellers.callcenter.update', 'id' => $seller->id]]) !!}
	...
	<div class="row">
		<div class="form-group col-md-4">
			<label for="area_seller">Tienda</label>
			<select name="area_seller" class="form-control" required>
				@foreach ($areas as $area)
					<option value="{{ $area->id }}" @if ($area->id == $seller->call_center_area_id){{'selected'}}@endif>
						{{ $area->name }}
					</option>
				@endforeach
			</select>
